...
void ObjHandle::add(char* filename, Camera camera) {
	...
	center(newobject, camera.getPosition()); //center object on camera
...
void ObjHandle::center(Object& obj, Transform camera) {
	...
	//std::cout << '(' << obj.getCenter().getX() << ", " << obj.getCenter().getY() << ", " << 
		    //obj.getCenter().getZ() << ", " << obj.getCenter().getW() << ")" << std::endl;
		    
	
	obj.setPosition(matrix.translate(center, matrix.fscale(center, -1.0)));
	
	//std::cout << '(' << obj.getPosition().getX() << ", " << obj.getPosition().getY() << ", " << 
		    //obj.getPosition().getZ() << ", " << obj.getPosition().getW() << ")" << std::endl;
	
	for (auto& t: obj.getTransformPos()) {
		t = matrix.translate(t, matrix.fscale(center, -1.0));
		t = matrix.translate(t, camera);
		//std::cout << '(' << t.getX() << ", " << t.getY() << ", " << 
		    //t.getZ() << ", " << t.getW() << ")" << std::endl;
		
	}
	
	obj.setPosition(matrix.translate(obj.getPosition(), camera));
	//obj.printObj();
}
